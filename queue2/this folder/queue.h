...
istream& operator>>(istream &in, queue<Y>& q)
{
    ...
    if(string(typeid(Y).name()) == "Ss")
        //while(getline(in,input,'\n'))
            {
                getline(in,input,'\n');        // get the user input
                stringstream strStream(input); // call stringstream to construct the object using user input as parameter
                strStream >> inputOther;       // dump the stream into the q
                q <<inputOther;

            }
    //q.enqueue(input);
    else if(string(typeid(Y).name()) == "c")
    {
        //
    }
    else
